...
BEGIN
  ...
  -- Calculate campaign cost (10-15% of sales)
  campaign_cost := final_sales * (0.10 + random() * 0.05);

  RETURN jsonb_build_object(
    'price', ROUND(final_price::numeric, 2),
    'quantity', final_quantity,
    'sales', ROUND(final_sales::numeric, 2),
    'cogs', ROUND(final_cogs::numeric, 2),
    'cost', ROUND(campaign_cost::numeric, 2),
    'visitors', visitors,
    'orders', final_quantity,
    'product_name', product_name,
    'product_type', product_type,
    'conversion_rate', ROUND((final_quantity::numeric / visitors::numeric * 100)::numeric, 2),
    'gross_profit', ROUND((final_sales - final_cogs)::numeric, 2),
    'baseline', ROUND((final_sales * 0.7)::numeric, 2) -- Baseline is 70% of actual sales
  );
END;
$$ LANGUAGE plpgsql;
...
WITH daily_totals AS (
  SELECT
    ...
    SUM((metrics->>'orders')::numeric) as total_orders,
    ...
    SUM((metrics->>'baseline')::numeric) as total_baseline,
...
SELECT
  'ALL' as platform,
  jsonb_build_object(
    'sales', ROUND(total_sales::numeric, 2),
    'orders', total_orders,
    'visitors', total_visitors,
    'cogs', ROUND(total_cogs::numeric, 2),
    'cost', ROUND(total_cost::numeric, 2),
    'gross_profit', ROUND(total_gross_profit::numeric, 2),
    'baseline', ROUND(total_baseline::numeric, 2),
