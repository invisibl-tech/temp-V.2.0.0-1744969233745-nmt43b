/*
  # Add product type to product costs

  1. Changes
    - Add product_type column to product_costs table
    - Update existing records with product types
    - Update mock data generation to include product types

  2. Security
    - Maintains existing RLS policies
*/

-- Add product_type column to product_costs
ALTER TABLE product_costs 
ADD COLUMN IF NOT EXISTS product_type text NOT NULL DEFAULT 'Clothing';

-- Update existing product costs with correct product types
UPDATE product_costs
SET product_type = 
  CASE 
    WHEN item_id LIKE 'DRESS%' THEN 'Dresses'
    WHEN item_id LIKE 'JEANS%' THEN 'Bottoms'
    WHEN item_id LIKE 'TSHIRT%' THEN 'Tops'
    WHEN item_id LIKE 'BLOUSE%' THEN 'Tops'
    WHEN item_id LIKE 'SKIRT%' THEN 'Bottoms'
    WHEN item_id LIKE 'JACKET%' THEN 'Outerwear'
    ELSE 'Clothing'
  END;
...
-- Update function to include product type
