/*
  # Create mock data for existing user
  ...
  2. Changes
    - Uses explicit table alias to avoid ambiguous column references
    - Ensures clean data by removing old test data first
    - Adds realistic mock data for all products
*/
...
BEGIN
  ...
  -- Clear existing product costs for this user
  DELETE FROM product_costs pc WHERE pc.user_id = v_user_id;

  -- Insert mock product costs
  INSERT INTO product_costs (item_id, name, size, color, cost, user_id)
  VALUES
    ('DRESS001', 'Summer Floral Dress', 'S', 'Blue', 450, v_user_id),
    ('DRESS002', 'Summer Floral Dress', 'M', 'Blue', 450, v_user_id),
    ('DRESS003', 'Summer Floral Dress', 'L', 'Blue', 500, v_user_id),
    ('JEANS001', 'Classic Denim Jeans', '28', 'Dark Blue', 600, v_user_id),
    ('JEANS002', 'Classic Denim Jeans', '30', 'Dark Blue', 600, v_user_id),
    ('JEANS003', 'Classic Denim Jeans', '32', 'Dark Blue', 600, v_user_id),
    ('TSHIRT001', 'Basic Cotton T-Shirt', 'S', 'White', 150, v_user_id),
    ('TSHIRT002', 'Basic Cotton T-Shirt', 'M', 'White', 150, v_user_id),
    ('TSHIRT003', 'Basic Cotton T-Shirt', 'L', 'White', 150, v_user_id),
    ('BLOUSE001', 'Silk Blouse', 'S', 'Pink', 800, v_user_id),
    ('BLOUSE002', 'Silk Blouse', 'M', 'Pink', 800, v_user_id),
    ('SKIRT001', 'A-Line Midi Skirt', 'S', 'Black', 550, v_user_id),
    ('SKIRT002', 'A-Line Midi Skirt', 'M', 'Black', 550, v_user_id),
    ('JACKET001', 'Denim Jacket', 'M', 'Light Blue', 900, v_user_id),
    ('JACKET002', 'Denim Jacket', 'L', 'Light Blue', 900, v_user_id);

  -- For each product
  FOR product_record IN SELECT item_id FROM product_costs pc WHERE pc.user_id = v_user_id LOOP
    -- For each platform
